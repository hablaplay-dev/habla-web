...
async function upsertFixture(fx) {
  const f = fx.fixture;
  const l = fx.league;
  const t = fx.teams;
  const g = fx.goals ?? {};
...
async function seedFixtures(date, leagues, season) {
...
async function syncLiveNow() {
  ...
  const { data: ms, error } = await sb.from("matches").select("id, af_fixture_id").not("af_fixture_id", "is", null).gte("start_time", tmin).lte("start_time", tmax);
  if (error) throw new Error(error.message);
  const fixtureIds = (ms ?? []).map((m)=>m.af_fixture_id);
  ...
  for (const ch of chunks){
    ...
    for (const fid of ch){
      const resp = await afFetch("/fixtures", {
        id: fid
      });
      const fx = resp?.[0];
      if (!fx) continue;
      ...
      const m = (ms || []).find((m)=>m.af_fixture_id === fid);
      if (m?.id) {
        try {
          await sb.rpc("score_match_live", {
            p_match_id: m.id
          });
        } catch (e) {
          console.warn("[sync-live] score_match_live failed for", m.id, e?.message || e);
        }
      }
      updated++;
    }
